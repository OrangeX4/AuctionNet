...
import torch
import pickle
from auctionbid.simul_bidding_env.strategy.base_bidding_strategy import BaseBiddingStrategy
import os


class MbrlComboMicroBiddingStrategy(BaseBiddingStrategy):
    """
    MBRL Strategy
    """

    def __init__(self, budget=100, name="MBRL-COMBOMICRO-PlayerStrategy", cpa=2, category=1):
        super().__init__(budget, name, cpa, category)
        file_name = os.path.dirname(os.path.realpath(__file__))
        dir_name = file_name
        model_path = os.path.join(dir_name, "official_agent", "mbrl_combomicro", "model", "policy_model",
                                  "best_policy.pt")
